          ...
          <div className="space-y-1">
            {databases.map((database) => (
              <div key={database} className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-colors ${
                    selectedDatabase === database
                      ? 'bg-[var(--add-bg)] border border-[var(--accent)]'
                      : 'hover:bg-[var(--panel)]'
                  }`}
                >
                  <span className="text-sm font-medium text-[var(--text-2)]">{database}</span>
                ...
                {collectionsByDatabase[database]?.length === 0 && (
                  <div className="pl-4 text-xs text-[var(--text-muted)] italic">
                ...
                {expandedDatabases.has(database) &&
                  collectionsByDatabase[database]?.length > 0 && (
                    <div className="pl-4 space-y-1 border-l border-[var(--border)] ml-3">
