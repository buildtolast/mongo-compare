...
async fn test_sample_limit_zero_suppresses_output() -> Result<()> {
    ...
    assert_eq!(
        sample_created.len(),
        0,
        "Should have no sample created documents"
    );
    assert_eq!(
        sample_deleted.len(),
        0,
...
async fn test_sample_limit_custom_returns_exact_count() -> Result<()> {
    ...
    assert_eq!(
        sample_created.len(),
        3,
        "Should have exactly 3 sample created documents"
    );
...
async fn test_sample_limit_exceeds_document_count() -> Result<()> {
    ...
    assert_eq!(
        sample_created.len(),
        1,
        "Should have 1 sample created document (capped at actual count)"
    );
...
async fn test_sample_limit_applies_to_all_categories() -> Result<()> {
    ...
    assert_eq!(
        sample_created.len(),
        1,
        "Should have 1 sample created document"
    );
    assert_eq!(
        sample_updated.len(),
        1,
        "Should have 1 sample updated document"
    );
    assert_eq!(
        sample_deleted.len(),
        0,
...
async fn test_sample_limit_default_five() -> Result<()> {
    ...
    assert_eq!(
        sample_created.len(),
        4,
        "Should have 4 sample created documents"
    );
